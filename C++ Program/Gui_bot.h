...
LRESULT PASCAL WndProc(HWND hWnd, UINT message,
	 WPARAM wParam, LPARAM lParam);
...
//		friend int SetTimer(UINT TimerID, UINT Time);  // Not implemented
        ...
        friend void DrawText(string S, int x1, int y1, int x2, int y2);
		...
		friend int wherex();
		friend int wherey();
        friend void updateScreen();
        friend void updateScreen(long x1, long y1, long x2, long y2);
		...
		friend void drawCenteredText(long x1, long y1, long x2, long y2);
        friend void DrawCenteredText(string S, int x1, int y1, int x2, int y2);
		MyGuiWindow();
...
void updateScreen()
{
    InvalidateRect(gui_out._hwnd, NULL, TRUE);
}

void updateScreen(long x1, long y1, long x2, long y2)
{
    const RECT b[4] = {x1,y1,x2,y2};
    InvalidateRect(gui_out._hwnd, b, TRUE);
}

void drawCenteredText(long x1, long y1, long x2, long y2)
{
    SetTextSize(300);
    const std::string a= "testddfvdfv dfvdf v";
    char *c = const_cast<char *>(a.c_str());
    //DWORD w = GetTextExtentPoint32A(gui_out._hdc, c, , );
    const RECT b[4] = {x1,y1,x2,y2};
    DrawTextExA(gui_out._hdc, c, a.size(), const_cast<LPRECT>(b), DT_SINGLELINE | DT_CENTER | DT_VCENTER, NULL);
...
void MyGuiWindow::open(HWND hwnd, HDC win_hdc)
{
	...
	_disp_xmax = rect.right/2 - 1;
	_disp_ymax = rect.bottom/2 - 1;
...
void DrawText(string S, int x1, int y1, int x2, int y2)
{
	SetTextColor(gui_out._hdc, RGB(gui_out.RedTextVal, gui_out.GreenTextVal, gui_out.BlueTextVal));
	HFONT hFont = CreateFont(gui_out.FontSize,0,0,0,FW_NORMAL,0,0,0,
			ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
...
// Below for Visual C++
	...
	//SetTextAlign(gui_out._hdc,TA_BASELINE | TA_CENTER | TA_UPDATECP);
	//TextOut(gui_out._hdc,P.x,P.x,S.c_str(),S.length());
    RECT b[4] = {x1,y1,x2,y2};
    DrawTextA(gui_out._hdc, S.c_str(), S.size(), b, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
// Above for Visual C++
...
void DrawCenteredText(string S, int x1, int y1, int x2, int y2)
{
    SetTextColor(gui_out._hdc, RGB(gui_out.RedTextVal, gui_out.GreenTextVal, gui_out.BlueTextVal));
    HFONT hFont;
    HFONT hOldFont;
    int fSize = gui_out.FontSize;
    int i = 450;
    bool width = false;
    for(i = 0; i <1000; i++) {
        SetTextSize(i);
        hFont = CreateFont(gui_out.FontSize, 0, 0, 0, FW_NORMAL, 0, 0, 0,
                           ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                           PROOF_QUALITY, gui_out.FontFamily | VARIABLE_PITCH, gui_out.FontName.c_str());
        tagSIZE size[2] = {0, 0};
        hOldFont = HFONT(SelectObject(gui_out._hdc, hFont));
        GetTextExtentPoint32A(gui_out._hdc, S.c_str(), S.size(), size);
        if(size->cx >x2-x1 || size->cy > y2-y1)
        {
            if(size->cx >x2-x1)
                width = true;
            fSize = i-1;
    ...
    SetTextSize(i);
    //SetTextColor(gui_out._hdc, RGB(gui_out.RedTextVal, gui_out.GreenTextVal, gui_out.BlueTextVal));
    //SetBkColor(gui_out._hdc, TRANSPARENT);
    //hFont = CreateFont(gui_out.FontSize,0,0,0,FW_NORMAL,0,0,0,
                            // ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                            // PROOF_QUALITY, gui_out.FontFamily | VARIABLE_PITCH, gui_out.FontName.c_str());
    //hOldFont = HFONT(SelectObject(gui_out._hdc, hFont));
...
//	 _InitEasyWin();	 cout << "Hello" << endl;
	 ...
	 hWnd = CreateWindow(
                    ...
                    WS_OVERLAPPED | WS_MINIMIZEBOX | WS_SYSMENU,  // The window's style.
                    sw/2 - width/2,                   // X position.
                    sh/2 - height/2,                   // Y position.
					width,                // Width.
					height,               // Height.
